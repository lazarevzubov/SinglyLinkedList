...
final class SinglyLinkedListCodableTests: XCTestCase {
    ...
    func testCodable() {
        ...
        let jsonString = "{\"firstNode\":{\"payload\":1,\"nextNode\":{\"payload\":2,\"nextNode\":{\"payload\":3}}}}"
    ...
    func testMalformedData() {
        ...
        XCTAssertThrowsError(try JSONDecoder().decode(SinglyLinkedList<Int>.self, from: erroneousJSONStringData1), "A field with a typo doesn't raise an error while deconding.")
        ...
        XCTAssertThrowsError(try JSONDecoder().decode(SinglyLinkedList<Int>.self, from: erroneousJSONStringData2),
                             "A JSON with a mismatched structure doesn't raise an error while deconding.")
    }

    func testExtraField() {
        let erroneousJSONString3 = "{\"firstNode\":{\"payload\":1,\"nextNode\":{\"payload\":2,\"nextNode\":{\"payload\":3}}},\"unknownField\":0}"
