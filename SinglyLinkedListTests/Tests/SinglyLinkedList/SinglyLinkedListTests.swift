...
final class SinglyLinkedListTests: XCTestCase {
    ...
    func testInsertAfter() {
        let node1 = Node(payload: 1)
        let list = SinglyLinkedList<Int>(firstNode: node1)

        let node2 = Node(payload: 2)
        ...
        let node0 = Node(payload: 0)
        let node4 = Node(payload: 4)
        ...
        let node5 = Node(payload: 5)
        let node6 = Node(payload: 6)
