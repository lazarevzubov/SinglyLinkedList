...
final class NodeTestCodable: XCTestCase {
    ...
    func testEncodeDecode() {
        ...
        let decodedNode = try! JSONDecoder().decode(Node<Int>.self, from: nodeData)
        ...
        assertNoThrow(try JSONDecoder().decode(Node<Int>.self, from: jsonStringData), "JSON decoder thrown an error while decoding a knowingly valid Node data.") {
        ...
        XCTAssertThrowsError(try JSONDecoder().decode(Node<Int>.self, from: erroneousJSONStringData1), "A field with a typo doesn't raise an error while deconding.")

        let erroneousJSONString2 = "{\"payload\":1,\"nextNode\":{}}"
        let erroneousJSONStringData2 = erroneousJSONString2.data(using: .utf8)!
        XCTAssertThrowsError(try JSONDecoder().decode(Node<Int>.self, from: erroneousJSONStringData2), "A JSON with a mismatched structure doesn't raise an error while deconding.")
        ...
        XCTAssertNoThrow(try JSONDecoder().decode(Node<Int>.self, from: erroneousJSONStringData3), "A JSON with an extra field raised an error.")
    }
